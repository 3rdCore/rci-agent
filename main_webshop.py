...
from webshop.web_agent_site.envs import WebAgentSiteEnv
from webshop.web_agent_site.utils import DEBUG_PROD_SIZE
...
def webshop(opt):
    env = WebAgentSiteEnv(
        observation_mode="html", render=False, num_products=DEBUG_PROD_SIZE
    )
    ...
    for _ in tqdm(range(opt.num_episodes), desc="Episodes", leave=False):
        ...
        # states = env.reset()
        llm_agent.set_goal(env.instruction_text.replace("Instruction: ", ""))
        # llm_agent.set_goal(states[0].utterance)

        # html_state = get_html_state(opt, states)
        html_state = env.observation
        ...
        for _ in range(step):
            ...
            try:
                ...
                )  # this is needed to perform actions

                # miniwob_action = llm_agent.convert_to_miniwob_action(instruction)

                html_state, reward, done, _ = env.step(instruction)
                llm_agent.save_action(str(instruction))
            except Exception as e:
            ...
            if done:  # or llm_agent.check_finish_plan():
                break
