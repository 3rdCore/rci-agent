...
from webshop.web_agent_site.engine.engine import parse_action, END_BUTTON
...
class WebAgentSiteEnv(gym.Env):
    ...
    def __init__(self, observation_mode="html", **kwargs):
        ...
        print("Using path {}".format(join(dirname(abspath(__file__)), "chromedriver")))
        service = Service(join(dirname(abspath(__file__)), "chromedriver"))
        service = Service(binary_path)  ## Overwrite chromedriver path
        options = Options()
    ...
    def step(self, action):
        ...
        if (
            action == "clickxpath //input[@id='search_input']"
        ):  # Rim added this because RCI decomposes search action into 2 strps slick on the search form then type
            action_name = "invalid"
            action_arg = "Search"
